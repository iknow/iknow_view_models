...
class ActiveRecordViewModel
  ...
  class UpdateData
    ...
    def visit_association(name, value, empty:, map:, reduce:)
      ...
      when association_data.collection?
        if association_data.shared?
          value.map(&map).inject(empty.(), &reduce)
        else
          value.update_datas.map(&map).inject(empty.(), &reduce)
        end
      else
        map.(value)
      end
    ...
    def visit_association_to_hash(name, value, &map)
      visit_association(
        name, value,
        empty:  ->() { {} },
        reduce: ->(acc, data) { acc.deep_merge(data) },
        map:    map)
    end
    ...
    def updated_associations(referenced_updates)
      ...
      associations.each do |assoc_name, assoc_update|
        deps[assoc_name] = visit_association_to_hash(assoc_name, assoc_update) do |update_data|
          update_data.updated_associations(referenced_updates)
        end
      end

      referenced_associations.each do |assoc_name, reference|
        deps[assoc_name] = visit_association_to_hash(assoc_name, reference) do |ref|
    ...
    def association_dependencies(referenced_updates)
      ...
      associations.each do |assoc_name, assoc_update|
        ...
        deps[association_data.direct_reflection.name.to_s] =
          visit_association_to_hash(assoc_name, assoc_update) do |update_data|
      ...
      referenced_associations.each do |assoc_name, reference|
        ...
        referenced_deps =
          visit_association_to_hash(assoc_name, reference) do |ref|
            if association_data.polymorphic?
              {}
            else
